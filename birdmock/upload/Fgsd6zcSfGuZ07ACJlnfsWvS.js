...
const pro = {
  host: '', // 服务器ip地址或域名
  password: 'NewNari1009', // 密码，请勿将此密码上传至git服务器
  catalog: '/home/split_dist/dist', // 前端文件压缩目录
  port: 22, // 服务器ssh连接端口号
...
async function sshUpload(sshConfig, fileName) {
  ...
  await sshCon.execSsh(`rm -rf /home/split_dist/dist`);
  console.log('----删除dist包成功----');

  await sshCon.execSsh(`mkdir /home/split_dist/dist`);
  console.log('----新建dist文件夹成功----');
  ...
  // const linkUrl = 'http://60.60.60.30:8080/jenkins/job/restart_nginx/build?token=restart_nginx';
  // await sshCon.execSsh(`curl ${linkUrl}`);

  console.log('----重启nginx...----');
  const restartNginxRes = await sshCon.execSsh(`sh /home/restartNginx.sh`);
  if (!restartNginxRes || !restartNginxRes.success) {
    console.error('----重启nginx失败----');
    console.error(`----错误原因：${restartNginxRes.error}----`);
  }
