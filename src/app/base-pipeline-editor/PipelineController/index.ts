        ...
        switch (p.info.type) {
          case 'missingProperty':
            if (p.info.nodeID === nodeID) {
              ...
              nodeProblems.push(`属性 "${label}" 是必填项`);
            }
            break;
          case 'invalidProperty':
            if (p.info.nodeID === nodeID) {
              const property = p.info.property;
              const label = componentProperties[property].title;
              nodeProblems.push(`属性 "${label}" 无效: ${p.info.message}`);
            }
            break;
          case 'missingComponent':
            if (p.info.nodeID === nodeID) {
              nodeProblems.push(`找不到组件 "${p.info.op}"`);
            }
            break;
        }
      }
    ...
    if (pipeline !== undefined) {
      ...
      for (const [key, val] of Object.entries(
        ...
        if (
          val === undefined ||
          (val === null &&
            (app_data.component_parameters as object).hasOwnProperty(key))
        ) {
